...
switch ($mode)
{
	...
	case 'auth':
		...
		if ($auth = $_CLASS['core_auth']->make_options($module['auth']))
		{
		...
		$_CLASS['core_auth']->make_options($module['auth'], true);
...
function modules_admin()
{
    ...
    $result = $_CLASS['core_db']->sql_query('SELECT * FROM '.CORE_MODULES_TABLE);
