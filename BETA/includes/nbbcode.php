...
/*********************************************
  ...
  ********************************************
  ...
  Last modification notes:
    ...
    $Id: nbbcode.php,v 1.22 2004/05/17 21:11:32 djmaze Exp $
  ...
  todo:
    image
        ...
        VSPACE
        ALIGN
        http://www.faqs.org/docs/htmltut/images/_IMG_ALIGN.html

***********************************************************************/
if (eregi("bbcode.php",$_SERVER['PHP_SELF'])) {
...
function set_smilies($message, $url='') {
    ...
    if (!isset($orig)) {
        ...
        $orig = $repl = array();
        $sql = 'SELECT * FROM '.$prefix.'_bb2smilies';
        ...
        if (file_exists("language/bbcode/lang-$currentlang.php")) {
            include("language/bbcode/lang-$currentlang.php");
        } else {
            include("language/bbcode/lang-$language.php");
        }
...
{
    global $db, $board_config, $template, $images, $theme, $phpEx, $phpbb_root_path;
    global $user_ip, $session_length, $starttime;
    global $userdata;
    global $smilies_path, $prefix;
    global $language, $currentlang, $MAIN_CFG;
    if (file_exists("language/bbcode/lang-$currentlang.php")) {
        include("language/bbcode/lang-$currentlang.php");
    } else {
        include("language/bbcode/lang-$language.php");
    ...
    if (defined('CPG_NUKE')) {
        ...
        $url = getlink("coppermine&amp;mode=smilies&amp;field=$field&amp;form=$form");
    }
...
//        $www_location = getenv('HTTP_HOST') . $path;
        ...
        <base href="http://'.getenv('HTTP_HOST').$MAIN_CFG['server']['path'].'"></head><body>
...
<table width="100" border="0" cellspacing="1" cellpadding="4" class="forumline">
  <tr>
    ...
    $sql = "SELECT emoticon, code, smile_url FROM ".$prefix."_bb2smilies ORDER BY smile_id";
    if ($result = $db->sql_query($sql)) {
        $num_smilies = 0;
        $rowset = array();
...
function smilies_table2($mode, $field="message", $form="post")
{
    global $db , $smilies_path, $prefix, $language, $currentlang, $MAIN_CFG;;
    if (file_exists("language/bbcode/lang-$currentlang.php")) {
        include("language/bbcode/lang-$currentlang.php");
    } else {
        include("language/bbcode/lang-$language.php");
    ...
    if (defined('CPG_NUKE')) {
        $url = getlink("smilies2&amp;field=$field&amp;form=$form");
    } else {
        $url = getlink("coppermine&amp;mode=smilies2&amp;field=$field&amp;form=$form");
    }
...
// process the smiley description'
        ...
        if ($num_smilies) {
            ...
            while (list($smile_url, $data) = @each($rowset)) {
                ...
                $content .= '<td><img src="http://'. getenv('HTTP_HOST') .$MAIN_CFG['server']['path']. $smilies_path . $smile_url . '" border="0" alt="'. $data['emoticon'].'" title="' .$data['emoticon'].'" /></td>';
                $s_colspan = max($s_colspan, $col + 1);
...
//        $text = preg_replace("#\[img\]((ht|f)tp://)([^ \?&=\"\n\r\t<]*?(\.(jpg|jpeg|gif|png|php)))\[/img\]#sie", "'[img]\\1' . str_replace('admin.php', ' ', '\\2') . '[/img]'", $text);
//        $patterns[] = "#\[img\]((ht|f)tp://)([^ \?&=\"\n\r\t<]*?(\.(jpg|jpeg|gif|png)))\[/img\]#si";
//        $replacements[] = "'<img src=\"\\1' . str_replace('admin.php', 'nothing', '\\3') . '\" border=\"0\" />'";
...
//        $replacements[] = "<img src=\"\\1\" border=\"0\" />";
        ...
        $replacements[] = "'<img src=\"\\1' . str_replace('admin.php', 'lozer.txt', '\\3') . '\" border=\"0\" />'";
