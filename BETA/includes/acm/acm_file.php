...
// -------------------------------------------------------------
//
// $Id: acm_file.php,v 1.15 2004/05/26 18:55:26 acydburn Exp $
//
...
class acm
{
	...
	function save($thefile=false, $thedata=false, $name=false) 
	{
		if (!$this->is_modified && !$thefile)
		{
			return;
		}
		
		$custom = ($thefile) ? true : false;
		
		if ($custom) {
		
			$data[$name] = $thedata;
			$expire[$name] = time() + 31536000;
			$thefile = $thefile;
			
		} else {
		
			$data = $this->vars;
			$expire = $this->var_expires;
			$thefile = 'data_global';
		
		}
		
		global $phpEx;
		$file = '<?php $this->vars=' . $this->format_array($data) . ";\n\$this->var_expires=" . $this->format_array($expire) . ' ?>';

		if ($fp = @fopen($this->cache_dir . $thefile . ".$phpEx", 'wb'))
		{
			@flock($fp, LOCK_EX);
			fwrite($fp, $file);
			@flock($fp, LOCK_UN);
	...
	{
	global $_CLASS, $MAIN_CFG, $config, $phpEx;
	
	// Grab global variables, re-cache if necessary
	if ($config = $_CLASS['cache']->get('config'))
	{
		$sql = 'SELECT * 
			FROM ' . CONFIG_TABLE . '
			WHERE is_dynamic = 1';
	...
	else
	{
		...
		$_CLASS['cache']->put('config', $cached_config);
		unset($cached_config);
	}
	
	if (!($MAIN_CFG = $_CLASS['cache']->get('MAIN_CFG')))
	{
		$MAIN_CFG = array();
		...
		$_CLASS['cache']->put('MAIN_CFG', $MAIN_CFG);
	}
	
	
	// Tidy the cache
	if (method_exists($_CLASS['cache'], 'tidy') && time() - $config['cache_gc'] > $config['cache_last_gc'])
	{
		$_CLASS['cache']->tidy();
