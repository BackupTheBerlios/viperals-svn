...
class blocks
{
	...
	function check_side($side) {
		...
		if (CPG_NUKE == 'Admin' && $side != BLOCK_LEFT)
		{
			return false;
		} elseif (CPG_NUKE == 'Admin')  {
			return true;
		}
	...
	function display($position) {
		...
		foreach($this->blocks[$position] AS $this->blocksrow) {
			...
			if ($this->blocksrow['expires']) {
				if (time() > $this->blocksrow['expires']) {
					//make this so it updates all that expired.
					$db->sql_query('UPDATE '.BLOCKS_TABLE." SET active='0', expires='0' WHERE bid='".$this->blocksrow['bid']."' limit 1");
				}
			 }
